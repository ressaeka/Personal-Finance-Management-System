...
} = await import('../../src/service/auth.js');
const {
  findUserByEmail,
  findUserByUsername,
  createUsers,
  findUserById,
  updateUserById,
} = await import('../../src/models/auth.js');
const { hashPassword, comparePassword } =
  await import('../../src/utils/bcrypt.js');
...
describe('AUTH SERVICE TESTS', () => {
  ...
  describe('LOGIN SERVICE', () => {
    describe('Validation Tests', () => {
      test('should throw error if username is missing', async () => {
        await expect(loginService({ password: 'Test123' })).rejects.toThrow(
          'Username dan Password wajib diisi',
        );
      });

      test('should throw error if password is missing', async () => {
        await expect(loginService({ username: 'testuser' })).rejects.toThrow(
          'Username dan Password wajib diisi',
        );
      });
    });
  ...
  describe('GET PROFILE SERVICE', () => {
    ...
    test('should throw error if user not found', async () => {
      ...
      await expect(getProfileService({ id_user: 999 })).rejects.toThrow(
  ...
  describe('UPDATE USER BY ID SERVICE', () => {
    ...
    describe('Duplicate Check Tests', () => {
      test('should throw error if email already taken by another user', async () => {
        ...
        await expect(
          updateUserByIdService(1, 'user', 'taken@b.com', 'Test123!x'),
      ...
      test('should throw error if username already taken by another user', async () => {
        ...
        await expect(
          updateUserByIdService(1, 'taken', 'a@b.com', 'Test123!x'),
    ...
    describe('Success Tests', () => {
      ...
      test('should throw error if user to update not found', async () => {
        ...
        await expect(
          updateUserByIdService(999, 'user', 'a@b.com', 'Test123!x'),
