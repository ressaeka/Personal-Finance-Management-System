...
describe('LAPORAN SERVICE TESTS', () => {
  ...
  describe('GENERATE LAPORAN SERVICE', () => {
    describe('Validation Tests', () => {
      test('should throw error if id_user is missing', async () => {
        await expect(generateLaporanService(null, 2024, 6)).rejects.toThrow(
          'User tidak valid',
        );
      });

      test('should throw error if id_user is not a positive integer', async () => {
        await expect(generateLaporanService(-1, 2024, 6)).rejects.toThrow(
          'User tidak valid',
        );
      });

      test('should throw error if tahun is missing', async () => {
        await expect(generateLaporanService(1, null, 6)).rejects.toThrow(
          'Tahun dan bulan harus diisi',
        );
      });

      test('should throw error if bulan is missing', async () => {
        await expect(generateLaporanService(1, 2024, null)).rejects.toThrow(
          'Tahun dan bulan harus diisi',
        );
      });

      test('should throw error if tahun is not a number', async () => {
        await expect(generateLaporanService(1, 'abc', 6)).rejects.toThrow(
          'Tahun tidak valid',
        );
      });

      test('should throw error if tahun is below 2000', async () => {
        await expect(generateLaporanService(1, 1999, 6)).rejects.toThrow(
          'Tahun tidak valid',
        );
      });

      test('should throw error if tahun is above 2100', async () => {
        await expect(generateLaporanService(1, 2101, 6)).rejects.toThrow(
          'Tahun tidak valid',
        );
      });

      test('should throw error if bulan is below 1', async () => {
        await expect(generateLaporanService(1, 2024, -1)).rejects.toThrow(
          'Bulan tidak valid',
        );
      });

      test('should throw error if bulan is above 12', async () => {
        await expect(generateLaporanService(1, 2024, 13)).rejects.toThrow(
          'Bulan tidak valid',
        );
      });
    });
  ...
  describe('GET ALL LAPORAN SERVICE', () => {
    describe('Validation Tests', () => {
      test('should throw error if id_user is missing', async () => {
        await expect(getAllLaporanService(null)).rejects.toThrow(
          'User tidak valid',
        );
      });

      test('should throw error if id_user is not a positive integer', async () => {
        await expect(getAllLaporanService('abc')).rejects.toThrow(
          'User tidak valid',
        );
      });
    });
  ...
  describe('GET LAPORAN BY PERIODE SERVICE', () => {
    describe('Validation Tests', () => {
      test('should throw error if id_user is missing', async () => {
        await expect(
          getLaporanByPeriodeService(null, '2024-06'),
        ).rejects.toThrow('User tidak valid');
      });

      test('should throw error if periode is missing', async () => {
        await expect(getLaporanByPeriodeService(1, null)).rejects.toThrow(
          'Periode harus diisi',
        );
      });

      test('should throw error if periode format is invalid', async () => {
        await expect(getLaporanByPeriodeService(1, '2024/06')).rejects.toThrow(
          'Format periode tidak valid',
        );
      });

      test('should throw error if periode format is incomplete', async () => {
        await expect(getLaporanByPeriodeService(1, '2024')).rejects.toThrow(
          'Format periode tidak valid',
        );
      });
    });
  ...
  describe('REKAP OTOMATIS SERVICE', () => {
    describe('Validation Tests', () => {
      test('should throw error if id_user is missing', async () => {
        await expect(rekapOtomatisService(null, '2024-06-15')).rejects.toThrow(
          'User tidak valid',
        );
      });

      test('should throw error if tanggal is missing', async () => {
        await expect(rekapOtomatisService(1, null)).rejects.toThrow(
          'Tanggal harus diisi',
        );
      });

      test('should throw error if tanggal format is invalid', async () => {
        await expect(rekapOtomatisService(1, '15-06-2024')).rejects.toThrow(
          'Format tanggal tidak valid',
        );
      });

      test('should throw error if tanggal is not a real date', async () => {
        await expect(rekapOtomatisService(1, '2024-13-01')).rejects.toThrow(
          'Format tanggal tidak valid',
        );
      });
    });
  ...
  describe('DELETE LAPORAN SERVICE', () => {
    describe('Validation Tests', () => {
      test('should throw error if id_laporan is missing', async () => {
        await expect(deleteLaporanService(null, 1)).rejects.toThrow(
          'ID laporan tidak valid',
        );
      });

      test('should throw error if id_laporan is not a positive integer', async () => {
        await expect(deleteLaporanService('abc', 1)).rejects.toThrow(
          'ID laporan tidak valid',
        );
      });

      test('should throw error if id_user is missing', async () => {
        await expect(deleteLaporanService(1, null)).rejects.toThrow(
          'User tidak valid',
        );
      });

      test('should throw error if id_user is not a positive integer', async () => {
        await expect(deleteLaporanService(1, 'abc')).rejects.toThrow(
          'User tidak valid',
        );
      });
    });
    ...
    describe('Error Handling Tests', () => {
      test('should throw 404 if laporan not found', async () => {
        ...
        await expect(deleteLaporanService(999, 1)).rejects.toThrow(
          'Laporan tidak ditemukan',
        );
      });
    });
