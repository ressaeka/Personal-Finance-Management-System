...
const { categoryService, getAllCategoryService, getCategoryByIdService, updateCategoryService, deleteCategoryService } = await import("../../src/service/category.js");
const { createCategory, getAllCategory, getCategoryById, updateCategory, deleteCategory } = await import("../../src/models/category.js");
...
describe("CATEGORY SERVICE TESTS", () => {
    ...
    describe("GET CATEGORY BY ID SERVICE", () => {
        describe("Validation Tests", () => {
            test("should throw error if id_category is missing", async () => {
                await expect(getCategoryByIdService(null, 1))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_category is not a number", async () => {
                await expect(getCategoryByIdService("abc", 1))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_category is negative", async () => {
                await expect(getCategoryByIdService(-1, 1))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_user is missing", async () => {
                await expect(getCategoryByIdService(1, null))
                    .rejects.toThrow("User tidak ditemukan");
            });
        });
        ...
        describe("Success and Not Found Tests", () => {
            ...
            test("should throw error if category not found", async () => {
                ...
                await expect(getCategoryByIdService(999, 1))
            ...
            test("should handle database error", async () => {
                ...
                await expect(getCategoryByIdService(1, 1)).rejects.toThrow("Database error");
            });
        });
    });
    ...
    describe("UPDATE CATEGORY SERVICE", () => {
        describe("Validation Tests", () => {
            test("should throw error if id_category is missing", async () => {
                await expect(updateCategoryService(null, 1, "Makanan", "pengeluaran"))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_category is not a number", async () => {
                await expect(updateCategoryService("abc", 1, "Makanan", "pengeluaran"))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_user is missing", async () => {
                await expect(updateCategoryService(1, null, "Makanan", "pengeluaran"))
                    .rejects.toThrow("User tidak ditemukan");
            });

            test("should throw error if nama_category is too short", async () => {
                await expect(updateCategoryService(1, 1, "ab", "pengeluaran"))
                    .rejects.toThrow("Nama category minimal 3 karakter");
            });

            test("should throw error if tipe is invalid", async () => {
                await expect(updateCategoryService(1, 1, "Makanan", "investasi"))
        ...
        describe("Success and Not Found Tests", () => {
            test("should update category successfully", async () => {
                ...
                const result = await updateCategoryService(1, 1, "Minuman", "pengeluaran");

                expect(result).toBeDefined();
                expect(result.nama_category).toBe("Minuman");
                expect(updateCategory).toHaveBeenCalledWith(1, 1, "Minuman", "pengeluaran");
            });

            test("should throw error if category to update not found", async () => {
                getCategoryById.mockResolvedValue(null);

                await expect(updateCategoryService(999, 1, "Makanan", "pengeluaran"))
                    .rejects.toThrow("Category tidak ditemukan");
            });
            ...
            test("should throw error if update returns null", async () => {
                ...
                await expect(updateCategoryService(1, 1, "Makanan", "pengeluaran"))
    ...
    describe("DELETE CATEGORY SERVICE", () => {
        describe("Validation Tests", () => {
            test("should throw error if id_category is missing", async () => {
                await expect(deleteCategoryService(null, 1))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_category is not a number", async () => {
                await expect(deleteCategoryService("abc", 1))
                    .rejects.toThrow("ID category tidak valid");
            });

            test("should throw error if id_user is missing", async () => {
                await expect(deleteCategoryService(1, null))
                    .rejects.toThrow("User tidak ditemukan");
            });
        });
        ...
        describe("Success and Not Found Tests", () => {
            ...
            test("should throw error if category to delete not found", async () => {
                ...
                await expect(deleteCategoryService(999, 1))
                    .rejects.toThrow("Category tidak ditemukan atau sudah dihapus");
            });

            test("should handle database error", async () => {
                deleteCategory.mockRejectedValue(new Error("Database error"));

                await expect(deleteCategoryService(1, 1)).rejects.toThrow("Database error");
            });
        });
    });
});
