...
const {
  ...
  getAllTransaksiService,
  getTransaksiByIdService,
  updateTransaksiService,
  deleteTransaksiService,
} = await import("../../src/service/transaksi.js");
const { createTransaksi, getAllTransaksi, getTransaksiById, updateTransaksi, deleteTransaksi } =
  await import("../../src/models/transaksi.js");
const { getCategoryById } = await import("../../src/models/category.js");
const { findUserById } = await import("../../src/models/auth.js");

describe("TRANSAKSI SERVICE TESTS", () => {
  beforeEach(() => {
    jest.resetAllMocks();
    findUserById.mockResolvedValue({ id_user: 1, username: "test" });
    getCategoryById.mockResolvedValue({ id_category: 1, nama_category: "Test", tipe: "pemasukan" });
  });

  describe("CREATE TRANSAKSI SERVICE", () => {
    describe("Validation Tests", () => {
      test("should throw error if id_user is missing", async () => {
        await expect(createTransaksiService(null, 1, 10000, "Test", "2024-06-15")).rejects.toThrow(
          "ID user tidak valid",
        );
      });
      ...
      test("should throw error if id_category is missing", async () => {
        await expect(
          createTransaksiService(1, null, 10000, "Test", "2024-06-15"),
        ).rejects.toThrow("ID category tidak valid");
      });

      test("should throw error if id_category is not a number", async () => {
        await expect(
          createTransaksiService(1, "abc", 10000, "Test", "2024-06-15"),
        ).rejects.toThrow("ID category tidak valid");
      });
      ...
      test("should throw error if jumlah is missing", async () => {
        ...
        await expect(
          createTransaksiService(1, 1, undefined, "Test", "2024-06-15"),
        ).rejects.toThrow("Jumlah harus berupa angka positif");
      });
      ...
      test("should reject jumlah of 0", async () => {
        getCategoryById.mockResolvedValue({ id_category: 1, tipe: "pemasukan" });
        await expect(
          createTransaksiService(1, 1, 0, "Test", "2024-06-15"),
        ).rejects.toThrow("Jumlah harus berupa angka positif");
      });
      ...
      test("should throw error if tanggal format is invalid", async () => {
        getCategoryById.mockResolvedValue({ id_category: 1, tipe: "pemasukan" });
        await expect(
          createTransaksiService(1, 1, 10000, "Test", "15-06-2024"),
    ...
    describe("Category Check Tests", () => {
      test("should throw error if category not found", async () => {
        ...
        await expect(
          createTransaksiService(1, 999, 10000, "Test", "2024-06-15"),
    ...
    describe("Success Tests", () => {
      ...
      test("should handle database error", async () => {
        ...
        await expect(
          createTransaksiService(1, 1, 50000, "Test", "2024-06-15"),
  ...
  describe("GET ALL TRANSAKSI SERVICE", () => {
    test("should throw error if id_user is missing", async () => {
      await expect(getAllTransaksiService(null)).rejects.toThrow("ID user tidak valid");
    });
  ...
  describe("GET TRANSAKSI BY ID SERVICE", () => {
    describe("Validation Tests", () => {
      test("should throw error if id_transaksi is missing", async () => {
        await expect(getTransaksiByIdService(null, 1)).rejects.toThrow("ID transaksi tidak valid");
      });

      test("should throw error if id_transaksi is not a number", async () => {
        await expect(getTransaksiByIdService("abc", 1)).rejects.toThrow("ID transaksi tidak valid");
      });

      test("should throw error if id_transaksi is negative", async () => {
        await expect(getTransaksiByIdService(-1, 1)).rejects.toThrow("ID transaksi tidak valid");
      });

      test("should throw error if id_user is missing", async () => {
        await expect(getTransaksiByIdService(1, null)).rejects.toThrow("ID user tidak valid");
      });
    });
    ...
    describe("Success and Not Found Tests", () => {
      ...
      test("should throw 404 if transaksi not found", async () => {
        ...
        await expect(getTransaksiByIdService(999, 1)).rejects.toThrow("Transaksi tidak ditemukan");
      });
  ...
  describe("UPDATE TRANSAKSI SERVICE", () => {
    describe("Validation Tests", () => {
      test("should throw error if id_transaksi is missing", async () => {
        await expect(
          updateTransaksiService(null, 1, 1, 50000, "Test", "2024-06-15"),
        ).rejects.toThrow("ID transaksi tidak valid");
      });

      test("should throw error if id_user is missing", async () => {
        await expect(
          updateTransaksiService(1, null, 1, 50000, "Test", "2024-06-15"),
      ...
      test("should throw error if id_category is invalid", async () => {
        ...
        getCategoryById.mockResolvedValue({ id_category: 1, tipe: "pemasukan" });
        await expect(
          updateTransaksiService(1, 1, "abc", 50000, "Test", "2024-06-15"),
        ).rejects.toThrow("ID category tidak valid");
      });
      ...
      test("should throw error if jumlah is missing", async () => {
        ...
        getCategoryById.mockResolvedValue({ id_category: 1, tipe: "pemasukan" });
        await expect(
          updateTransaksiService(1, 1, 1, undefined, "Test", "2024-06-15"),
        ).rejects.toThrow("Jumlah harus berupa angka positif");
      });
      ...
      test("should throw error if tanggal format is invalid", async () => {
        ...
        getCategoryById.mockResolvedValue({ id_category: 1, tipe: "pemasukan" });
        await expect(
          updateTransaksiService(1, 1, 1, 50000, "Test", "invalid-date"),
        ).rejects.toThrow("Format tanggal tidak valid");
      });
    });
    ...
    describe("Not Found Tests", () => {
      test("should throw 404 if transaksi to update not found", async () => {
        ...
        await expect(
          updateTransaksiService(999, 1, 1, 50000, "Test", "2024-06-15"),
      ...
      test("should throw error if category not found", async () => {
        ...
        await expect(
          updateTransaksiService(1, 1, 999, 50000, "Test", "2024-06-15"),
        ).rejects.toThrow("Category tidak ditemukan");
      });
    });
    ...
    describe("Success Tests", () => {
      test("should update transaksi successfully", async () => {
        ...
        getCategoryById.mockResolvedValue({
          id_category: 2,
          nama_category: "Belanja",
          tipe: "pengeluaran",
        });
        updateTransaksi.mockResolvedValue(updatedTransaksi);

        const result = await updateTransaksiService(
          1,
          1,
          2,
          75000,
      ...
      test("should throw error if update returns null", async () => {
        ...
        await expect(
          updateTransaksiService(1, 1, 1, 50000, "Test", "2024-06-15"),
  ...
  describe("DELETE TRANSAKSI SERVICE", () => {
    describe("Validation Tests", () => {
      test("should throw error if id_transaksi is missing", async () => {
        await expect(deleteTransaksiService(null, 1)).rejects.toThrow("ID transaksi tidak valid");
      });

      test("should throw error if id_transaksi is not a number", async () => {
        await expect(deleteTransaksiService("abc", 1)).rejects.toThrow("ID transaksi tidak valid");
      });

      test("should throw error if id_user is missing", async () => {
        await expect(deleteTransaksiService(1, null)).rejects.toThrow("ID user tidak valid");
      });
    });
    ...
    describe("Success and Not Found Tests", () => {
      ...
      test("should throw 404 if transaksi to delete not found", async () => {
        ...
        await expect(deleteTransaksiService(999, 1)).rejects.toThrow(
