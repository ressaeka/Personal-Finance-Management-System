...
export const createTransaksi = async (req, res, next) => {
  try {
    const transaksi = await createTransaksiService(req.user.id, req.body);
...
export const findAllTransaksi = async (req, res, next) => {
  try {
    const transaksi = await findAllTransaksiService(req.user.id, {
      page: req.query.page,
...
export const findTransaksiById = async (req, res, next) => {
  try {
    const transaksi = await findTransaksiByIdService(Number(req.params.id), req.user.id);
...
export const updateTransaksi = async (req, res, next) => {
  try {
    const transaksi = await updateTransaksiService(Number(req.params.id), req.user.id, req.body);
...
export const deleteTransaksi = async (req, res, next) => {
  try {
    const transaksi = await deleteTransaksiService(Number(req.params.id), req.user.id);
